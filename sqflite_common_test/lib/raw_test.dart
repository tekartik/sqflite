  ...
  group('raw', () {
    ...
    test('Concurrency 1', () async {
      ...
      Future action2() async {
        await db.transaction((txn) async {
          ...
          await step1.future;
          await txn.rawInsert(
              'INSERT INTO Test (name) VALUES (?)', <dynamic>['item 1']);
          step2.complete();
    ...
    test('Transaction recursive', () async {
      ...
      await db.transaction((txn) async {
        ...
        await txn.rawInsert(
            'INSERT INTO Test (name) VALUES (?)', <dynamic>['item 2']);
      });
      var afterCount =
          utils.firstIntValue(await db.rawQuery('SELECT COUNT(*) FROM Test'));
      expect(afterCount, 2);
    ...
    test('Transaction open twice', () async {
      ...
      await db.transaction((txn) async {
        await txn
            .rawInsert('INSERT INTO Test (name) VALUES (?)', <dynamic>['item']);
        var afterCount = utils
            .firstIntValue(await txn.rawQuery('SELECT COUNT(*) FROM Test'));
        expect(afterCount, 1);
    ...
    test('Demo', () async {
      ...
      count = await database.rawDelete(
          'DELETE FROM Test WHERE name = ?', <dynamic>['another name']);
      print('deleted: $count');
      expect(count, 1);
    ...
    test('Demo clean', () async {
      ...
      await database.transaction((txn) async {
        ...
        var id2 = await txn.rawInsert(
            ...
            <dynamic>['another name', 12345678, 3.1416]);
        print('inserted2: $id2');
      });
    ...
    test('BatchQuery', () async {
      ...
      var batch = db.batch();
      batch.execute('CREATE TABLE Test (id INTEGER PRIMARY KEY, name TEXT)');
      batch.rawInsert('INSERT INTO Test (name) VALUES (?)', <dynamic>['item1']);
    ...
    test('Batch', () async {
      ...
      // two insert
      batch = db.batch();
      batch.rawInsert('INSERT INTO Test (name) VALUES (?)', <dynamic>['item2']);
      batch.insert('Test', <String, dynamic>{'name': 'item3'});
      results = await batch.commit();
      expect(results, [2, 3]);
      ...
      batch.rawUpdate('UPDATE Test SET name = ? WHERE name = ?',
          <dynamic>['new_item', 'item1']);
      batch.update('Test', <String, dynamic>{'name': 'new_other_item'},
      ...
      // delete
      batch = db.batch();
      batch.rawDelete('DELETE FROM Test WHERE name = ?', <dynamic>['new_item']);
      batch.delete('Test',
      ...
      // No result
      batch = db.batch();
      batch.insert('Test', <String, dynamic>{'name': 'item'});
      batch.update('Test', <String, dynamic>{'name': 'new_item'},
    ...
    test('Batch in transaction', () async {
      ...
      await db.transaction((txn) async {
        ...
        batch2.rawInsert(
            'INSERT INTO Test (name) VALUES (?)', <dynamic>['item1']);
    ...
    test('text primary key', () async {
      ...
      var id = await db.insert('Test', <String, dynamic>{'name': 'test'});
      expect(id, 1);
    ...
    test('Reference query', () async {
      ...
      try {
        ...
        batch.rawInsert(
            'INSERT INTO Other (name) VALUES (?)', <dynamic>['other 1']);
